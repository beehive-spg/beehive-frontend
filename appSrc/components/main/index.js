// Main React component, that we'll import in `appSrc/app.js`
//
// Note a few points from this file:
//
// 1.  We're using the format `main/index.js` for this file, which means we
// can simply import 'appSrc/components/main', which will auto-default to index.js.
...
import GraphQLMessage from 'appSrc/components/graphql'
import { Home, Page, WhenNotFound } from 'appSrc/components/routes'
import ReduxCounter from 'appSrc/components/redux'
import Stats from 'appSrc/components/stats'
import Styles from 'appSrc/components/styles'
...
export default () => (
	<div>
		...
		<div className={css.hello}>
			<img appSrc={logo} alt="ReactQL" className={css.logo} />
		</div>
		<hr />
		<GraphQLMessage />
		<hr />
		<ul>
			...
			<li>
				<Link to="/old/path">Redirect from /old/path &#8594; /new/path</Link>
			</li>
		</ul>
		<hr />
		<Switch>
